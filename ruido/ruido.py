...
def main():
    global cam
    cam = input("Digite o caminho do diretorio da imagem a ser processada: ")
...
def ruido():
    ...
    g2 = float(input("Digite o valor desejado para operação com ruido gaussiano: "))
    ...
    plt.subplot(122), plt.imshow(ruido2mul, cmap='gray'),plt.title('Ruido Gaussiano com valor requerido')
    plt.xticks([]), plt.yticks([])
    plt.show()
    ...
    # RUIDO S&P
    prob = float(input("Digite o valor de ruido desejado para utlizar no filtro S&P: "))   #SCRIPT PARA VALOR S&P
    output = image.copy()
